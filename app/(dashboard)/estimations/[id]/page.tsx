...
export default async function EstimateDetailPage({
  ...
  searchParams: Promise<{ addSection?: string; addItem?: string }>
}) {
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="text-sm text-gray-500 mb-4">
        <Link href="/estimations" className="hover:text-gray-900">
          Dự toán
        </Link>
        ...
        <div className="flex items-center gap-3">
          <span
            className={
              ...
              (STATUS_COLOR[estimate.status] ?? 'bg-gray-100 text-gray-700')
            }
          >
          ...
          <form
            action={updateEstimateStatusAction.bind(
              null,
              id,
              estimate.status === 'draft' ? 'sent' : 'draft',
            )}
          >
            <button
              type="submit"
              className="text-xs border border-gray-200 px-3 py-1.5 rounded-sm hover:bg-gray-50"
            >
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <Link
            href={`?addSection=1`}
            className="text-xs text-gray-500 hover:text-gray-900 border border-gray-200 px-3 py-1.5 rounded-sm hover:bg-gray-50"
          >
            + Thêm nhóm
          </Link>
          {sections.length === 0 && (
            <Link
              href={`?addItem=__none__`}
              className="text-xs text-gray-500 hover:text-gray-900 border border-gray-200 px-3 py-1.5 rounded-sm hover:bg-gray-50"
            >
              + Thêm dòng nhanh
            </Link>
      ...
      {sp.addSection === '1' && (
        <form
          action={createEstimateSectionAction}
          className="bg-gray-50 border border-gray-200 rounded-sm p-4 mb-4 flex gap-3 items-center"
        >
          <input type="hidden" name="estimate_id" value={id} />
          <input
            name="name"
            placeholder="Tên nhóm hạng mục (VD: Phòng khách, Thi công, Vật liệu…)"
            autoFocus
            className="flex-1 border border-gray-200 rounded-sm px-3 py-2 text-sm focus:outline-none focus:border-gray-900"
          />
          <button
            type="submit"
            className="bg-gray-900 text-white px-4 py-2 rounded-sm text-sm"
          >
            Thêm
          </button>
          <Link
      ...
      {sections.length === 0 && items.filter((i) => !i.section_id).length === 0 && sp.addItem !== '__none__' && (
        <div className="border border-dashed border-gray-200 rounded-sm p-10 text-center text-gray-400 text-sm">
          ...
          <Link href="?addSection=1" className="underline">
            Thêm nhóm đầu tiên
          </Link>{' '}
          hoặc{' '}
          <Link href="?addItem=__none__" className="underline">
            thêm dòng nhanh
          </Link>
          .
        </div>
          ...
          <div key={section.id} className="mb-6">
            <div className="flex items-center justify-between bg-gray-50 border border-gray-100 rounded-t-sm px-4 py-2.5">
              ...
              <div className="flex items-center gap-4">
                ...
                <form action={deleteEstimateSectionAction.bind(null, section.id, id)}>
                  <button type="submit" className="text-xs text-gray-300 hover:text-red-500">
                    Xoá nhóm
                  </button>
                </form>
              </div>
            ...
            <div className="border border-t-0 border-gray-100 rounded-b-sm overflow-hidden">
              {sItems.length > 0 && (
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {sItems.map((item) => {
                      const lineTotal =
                        item.quantity * item.unit_price * (1 + item.markup_pct / 100)
                      return (
                        <tr
                          key={item.id}
                        ...
                        >
                          ...
                          <td className="px-3 py-2.5 text-right font-medium">
                            {new Intl.NumberFormat('vi-VN').format(Math.round(lineTotal))}
                          </td>
                          <td className="px-2 py-2.5">
                            <form
                              action={deleteEstimateItemAction.bind(null, item.id, id)}
                            >
                              <button
                                type="submit"
              ...
              {sp.addItem === section.id ? (
                <form
                  action={createEstimateItemAction}
                  className="flex items-end gap-2 p-3 bg-gray-50 border-t border-gray-100"
                >
                  <input type="hidden" name="estimate_id" value={id} />
                  ...
                  <div className="flex-1">
                    <input
                      ...
                      autoFocus
                      className="w-full border border-gray-200 rounded-sm px-2 py-1.5 text-xs focus:outline-none focus:border-gray-900"
                    />
                  </div>
                  <input
                    name="quantity"
                    ...
                    placeholder="SL"
                    defaultValue="1"
                    className="w-14 border border-gray-200 rounded-sm px-2 py-1.5 text-xs text-right"
                  />
                  <input
                    name="unit"
                    placeholder="ĐV"
                    defaultValue="cái"
                    className="w-12 border border-gray-200 rounded-sm px-2 py-1.5 text-xs"
                  />
                  <input
                    name="unit_price"
                    type="number"
                    placeholder="Đơn giá"
                    defaultValue="0"
                    className="w-32 border border-gray-200 rounded-sm px-2 py-1.5 text-xs text-right"
                  />
                  <input
                    name="markup_pct"
                    type="number"
                    placeholder="Markup%"
                    defaultValue="0"
                    className="w-16 border border-gray-200 rounded-sm px-2 py-1.5 text-xs text-right"
                  />
                  <button
                    type="submit"
                    className="bg-gray-900 text-white px-3 py-1.5 rounded-sm text-xs"
                  >
                    Thêm
                  </button>
                  <Link
      ...
      {/* Items without section (quick add) */}
                  ...
                  {items
                    ...
                    .map((item) => {
                      ...
                      return (
                        ...
                        >
                          <td className="px-4 py-2.5">{item.description}</td>
                          <td className="px-3 py-2.5 text-right text-gray-600 w-14">
                            {item.quantity}
                          </td>
                          <td className="px-3 py-2.5 text-gray-600 w-12">{item.unit}</td>
                          <td className="px-3 py-2.5 text-right text-gray-600 w-32">
                            {new Intl.NumberFormat('vi-VN').format(item.unit_price)}
                          </td>
                          <td className="px-3 py-2.5 text-right text-gray-600 w-16">
                            {item.markup_pct > 0 ? `+${item.markup_pct}%` : '—'}
                          </td>
                          <td className="px-3 py-2.5 text-right font-medium w-36">
                            {new Intl.NumberFormat('vi-VN').format(Math.round(lineTotal))}
                          </td>
